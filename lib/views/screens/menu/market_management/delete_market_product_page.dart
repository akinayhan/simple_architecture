...
import '../../../widgets/button_design/delete_material_button.dart';
import '../../../widgets/dialog_design/quick_alert_utils.dart';

class DeleteMarketProductPage extends StatefulWidget {
  final int companyId;
  final String token;

  const DeleteMarketProductPage({super.key, required this.companyId, required this.token});

  @override
  DeleteMarketProductPageState createState() => DeleteMarketProductPageState();
}

class DeleteMarketProductPageState extends State<DeleteMarketProductPage> {
  ...
  void _onPressedUpdateProduct(UpdateMarketProductViewModel updateMarketProductViewModel) async {
    if (_formKey.currentState!.validate()) {
      UpdateMarketProductModel updateMarketProduct = UpdateMarketProductModel(
        ...
        productPrice: _selectedMarketProductPrice!,
        status: false,
        deleted: true,
      );
